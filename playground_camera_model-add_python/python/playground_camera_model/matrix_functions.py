...
class Matrix_Functions:
    ...
    def homogeneous_transformation(cls, window):
        ...
        V_T_Polygon = cls.create_homogeneous_transformation_matrix(
            (window.get_polygon_system_translation_x() - 10000) / 1000.0,
            (window.get_polygon_system_translation_y() - 10000) / 1000.0,
            (window.get_polygon_system_translation_z() - 10000) / 1000.0,
            cls.DEG_TO_RAD(window.get_polygon_system_rotation_roll() / 10.0),
            cls.DEG_TO_RAD(window.get_polygon_system_rotation_pitch() / 10.0),
            cls.DEG_TO_RAD(window.get_polygon_system_rotation_yaw() / 10.0),
            window.get_polygon_system_scale()
        )

        V_T_Pyramide = cls.create_homogeneous_transformation_matrix(
            (window.get_pyramide_system_translation_x() - 10000) / 1000.0,
            (window.get_pyramide_system_translation_y() - 10000) / 1000.0,
            (window.get_pyramide_system_translation_z() - 10000) / 1000.0,
            cls.DEG_TO_RAD(window.get_pyramide_system_rotation_roll() / 10.0),
            cls.DEG_TO_RAD(window.get_pyramide_system_rotation_pitch() / 10.0),
            cls.DEG_TO_RAD(window.get_pyramide_system_rotation_yaw() / 10.0),
            window.get_pyramide_system_scale()
        )

        V_T_Cone = cls.create_homogeneous_transformation_matrix(
            (window.get_cone_system_translation_x() - 10000) / 1000.0,
            (window.get_cone_system_translation_y() - 10000) / 1000.0,
            (window.get_cone_system_translation_z() - 10000) / 1000.0,
            cls.DEG_TO_RAD(window.get_cone_system_rotation_roll() / 10.0),
            cls.DEG_TO_RAD(window.get_cone_system_rotation_pitch() / 10.0),
            cls.DEG_TO_RAD(window.get_cone_system_rotation_yaw() / 10.0),
            window.get_cone_system_scale()
        )

        return V_T_C, C_T_V, V_T_Cube, V_T_Polygon, V_T_Pyramide, V_T_Cone
